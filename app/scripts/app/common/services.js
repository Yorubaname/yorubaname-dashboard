...
angular.module('dashboardappApp').service('baseService', [
  ...
  '$localStorage',
  '$state',
  function (FileUploader, ENV, toastr, $localStorage, $state) {
    ...
    return function (options) {
      ...
      var uploader = new FileUploader(args), options = options || {}, fileType = options.fileType, maxUpload = options.maxUpload || 1, invalidFileMsg = options.invalidFileMsg || 'File type is not supported', uploadLimitMsg = options.uploadLimitMsg || 'You may only upload one file at a time', onError = options.onErrorMsg || 'An error occur while uploading the file, please retry', onComplete = options.onCompleteMsg || 'File upload completed successfully', errorCallback = options.errorCallback || function () {
